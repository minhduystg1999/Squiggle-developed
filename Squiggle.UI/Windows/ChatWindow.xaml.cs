...
namespace Squiggle.UI.Windows
{
    ...
    public partial class ChatWindow : StickyWindowBase, IChatWindow
    {
        ...
        internal void SetChatSession(IChat chat)
        {
            ...
            chatSession.LikeReceived += chatSession_LikeReceived;
        ...
        private void MuteConversation()
        {
            if (IsGroupChat)
            {
                ignore = !ignore;
                if (ignore)
                    chatTextBox.AddInfo(Translation.Instance.ChatWindow_Mute);
                else
                    chatTextBox.AddInfo(Translation.Instance.ChatWindow_Unmute);
            }
            else
            {
                PrimaryBuddy.IsIgnored = !PrimaryBuddy.IsIgnored;
        ...
        public void Like()
        {
            if (!EnsureChatSession() || !chatState.ChatStarted)
                return;

            if (chatState.CanSendLike)
            {
                chatTextBox.AddInfo(Translation.Instance.ChatWindow_YouSentLike);
                chatSession.Like();
                chatState.LikeSent();
                DoLikeAction();
            }
            else
                chatTextBox.AddError(Translation.Instance.ChatWindow_LikeError, String.Empty);
        }
        ...
        void ChangeTitle_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (ChangeTitleDialog.Show(ref title) == System.Windows.Forms.DialogResult.OK)
            {
                this.Title = title;
                titleChanged = true;
            }                
        ...
        void FlashWindow()
        {
            if ( (IsGroupChat && ignore) || (PrimaryBuddy.IsIgnored && !IsGroupChat) || SettingsProvider.Current.Settings.GeneralSettings.DoNotDisturb)
                return;
